...
/********* Serial port ********************************************/
boolean SIM800C_FONA::setBaudrate(uint16_t baud) {
  return sendCheckReply(F("AT+IPREX="), baud, ok_reply);
}

/********* Real Time Clock ********************************************/

boolean SIM800C_FONA::readRTC(uint8_t *year, uint8_t *month, uint8_t *date, uint8_t *hr, uint8_t *min, uint8_t *sec) {
  uint16_t v;
...
boolean SIM800C_FONA::enableRTC(uint8_t i) {
...
uint8_t SIM800C_FONA::unlockSIM(char *pin)
{
...
uint8_t SIM800C_FONA::getSIMCCID(char *ccid) {
  getReply(F("AT+CCID"));
...
uint8_t SIM800C_FONA::getIMEI(char *imei) {
  getReply(F("AT+GSN"));
...
uint8_t SIM800C_FONA::getNetworkStatus(void) {
  uint16_t status;
...
uint8_t SIM800C_FONA::getRSSI(void) {
  uint16_t reply;
...
boolean SIM800C_FONA::setSMSInterrupt(uint8_t i) {
  return sendCheckReply(F("AT+CFGRI="), i, ok_reply);
}

int8_t SIM800C_FONA::getNumSMS(void) {
  uint16_t numsms;
...
// printouts!
boolean SIM800C_FONA::readSMS(uint8_t i, char *smsbuff,
...
boolean SIM800C_FONA::sendSMS(char *smsaddr, char *smsmsg) {
  if (! sendCheckReply(F("AT+CMGF=1"), ok_reply)) return false;
...
boolean SIM800C_FONA::deleteSMS(uint8_t i) {
    if (! sendCheckReply(F("AT+CMGF=1"), ok_reply)) return false;
  // read an sms
...
boolean SIM800C_FONA::sendUSSD(char *ussdmsg, char *ussdbuff, uint16_t maxlen, uint16_t *readlen) {
...
boolean SIM800C_FONA::enableNetworkTimeSync(boolean onoff) {
  if (onoff) {
...
boolean SIM800C_FONA::enableNTPTimeSync(boolean onoff, FONAFlashStringPtr ntpserver) {
  if (onoff) {
...
boolean SIM800C_FONA::getTime(char *buff, uint16_t maxlen) {
...
boolean SIM800C_FONA::getGSMLoc(float *lat, float *lon) {
...
boolean SIM800C_FONA::TCPconnect(char *server, uint16_t port) {
  flushInput();
...
boolean SIM800C_FONA::TCPclose(void) {
  return sendCheckReply(F("AT+CIPCLOSE"), ok_reply);
}

boolean SIM800C_FONA::TCPconnected(void) {
...
boolean SIM800C_FONA::TCPsend(char *packet, uint8_t len) {
...
uint16_t SIM800C_FONA::TCPavailable(void) {
  uint16_t avail;
...
uint16_t SIM800C_FONA::TCPread(uint8_t *buff, uint8_t len) {
  uint16_t avail;
...
boolean SIM800C_FONA::HTTP_init() {
  return sendCheckReply(F("AT+HTTPINIT"), ok_reply);
}

boolean SIM800C_FONA::HTTP_term() {
...
boolean SIM800C_FONA::HTTP_para_end(boolean quoted) {
  if (quoted)
    mySerial->println('"');
  else
    mySerial->println();
...
boolean SIM800C_FONA::HTTP_data(uint32_t size, uint32_t maxTime) {
  flushInput();
...
boolean SIM800C_FONA::HTTP_readall(uint16_t *datalen) {
...
boolean SIM800C_FONA::HTTP_ssl(boolean onoff) {
...
void SIM800C_FONA::HTTP_POST_end(void) {
  HTTP_term();
}

void SIM800C_FONA::setUserAgent(FONAFlashStringPtr useragent) {
  this->useragent = useragent;
}

void SIM800C_FONA::setHTTPSRedirect(boolean onoff) {
  httpsredirect = onoff;
}

/********* HTTP HELPERS ****************************************/

boolean SIM800C_FONA::HTTP_setup(char *url) {
  // Handle any pending
  HTTP_term();
...
boolean SIM800C_FONA::expectReply(FONAFlashStringPtr reply,
                                   uint16_t timeout) {
  readline(timeout);
...
inline int SIM800C_FONA::available(void) {
  return mySerial->available();
}

inline size_t SIM800C_FONA::write(uint8_t x) {
  return mySerial->write(x);
}

inline int SIM800C_FONA::read(void) {
  return mySerial->read();
}

inline int SIM800C_FONA::peek(void) {
  return mySerial->peek();
}

inline void SIM800C_FONA::flush() {
  mySerial->flush();
}

void SIM800C_FONA::flushInput() {
...
uint16_t SIM800C_FONA::readRaw(uint16_t b) {
  uint16_t idx = 0;
...
uint8_t SIM800C_FONA::readline(uint16_t timeout, boolean multiline) {
  uint16_t replyidx = 0;
...
uint8_t SIM800C_FONA::getReply(char *send, uint16_t timeout) {
  flushInput();
...
uint8_t SIM800C_FONA::getReply(FONAFlashStringPtr send, uint16_t timeout) {
  flushInput();
...
uint8_t SIM800C_FONA::getReply(FONAFlashStringPtr prefix, char *suffix, uint16_t timeout) {
  flushInput();
...
// Send prefix, suffix, and newline. Return response (and also set replybuffer with response).
uint8_t SIM800C_FONA::getReply(FONAFlashStringPtr prefix, int32_t suffix, uint16_t timeout) {
  flushInput();
...
uint8_t SIM800C_FONA::getReply(FONAFlashStringPtr prefix, int32_t suffix1, int32_t suffix2, uint16_t timeout) {
  flushInput();
...
// Send prefix, ", suffix, ", and newline. Return response (and also set replybuffer with response).
uint8_t SIM800C_FONA::getReplyQuoted(FONAFlashStringPtr prefix, FONAFlashStringPtr suffix, uint16_t timeout) {
  flushInput();
...
boolean SIM800C_FONA::sendCheckReply(char *send, char *reply, uint16_t timeout) {
...
boolean SIM800C_FONA::sendCheckReply(FONAFlashStringPtr send, FONAFlashStringPtr reply, uint16_t timeout) {
	if (! getReply(send, timeout) )
		return false;
...
boolean SIM800C_FONA::sendCheckReply(char* send, FONAFlashStringPtr reply, uint16_t timeout) {
  if (! getReply(send, timeout) )
	  return false;
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}
...
boolean SIM800C_FONA::sendCheckReply(FONAFlashStringPtr prefix, char *suffix, FONAFlashStringPtr reply, uint16_t timeout) {
  getReply(prefix, suffix, timeout);
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, and newline.  Verify FONA response matches reply parameter.
boolean SIM800C_FONA::sendCheckReply(FONAFlashStringPtr prefix, int32_t suffix, FONAFlashStringPtr reply, uint16_t timeout) {
  getReply(prefix, suffix, timeout);
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, suffix2, and newline.  Verify FONA response matches reply parameter.
boolean SIM800C_FONA::sendCheckReply(FONAFlashStringPtr prefix, int32_t suffix1, int32_t suffix2, FONAFlashStringPtr reply, uint16_t timeout) {
  getReply(prefix, suffix1, suffix2, timeout);
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, ", suffix, ", and newline.  Verify FONA response matches reply parameter.
boolean SIM800C_FONA::sendCheckReplyQuoted(FONAFlashStringPtr prefix, FONAFlashStringPtr suffix, FONAFlashStringPtr reply, uint16_t timeout) {
  getReplyQuoted(prefix, suffix, timeout);
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}


boolean SIM800C_FONA::parseReply(FONAFlashStringPtr toreply,
...
// response.
boolean SIM800C_FONA::parseReplyQuoted(FONAFlashStringPtr toreply,
